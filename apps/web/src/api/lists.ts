...
export async function fetchLists(): Promise<GroceryList[]> {
  const res = await authJson<Data<{ lists: GroceryList[] }>>('/api/v1/lists');
  return res.data.lists;
}

export async function createList(name: string): Promise<GroceryList> {
  const res = await authJson<Data<{ list: GroceryList }>>('/api/v1/lists', {
    method: 'POST',
...
export async function deleteList(listId: number): Promise<void> {
  const res = await authFetch(`/api/v1/lists/${listId}`, { method: 'DELETE' });
  if (res.status === 204) {
    return;
  }
...
export async function fetchListDetail(listId: number): Promise<{
  ...
  const res = await authJson<Data<{ list: GroceryList; items: ListItem[] }>>(
    `/api/v1/lists/${listId}`,
  );
  return res.data;
}

export async function fetchListOptimization(listId: number): Promise<ListOptimizationResult> {
  const res = await authJson<Data<ListOptimizationResult>>(`/api/v1/lists/${listId}/optimize`);
  return res.data;
}
...
export async function createListItem(
  ...
  const res = await authJson<Data<{ item: ListItem }>>(`/api/v1/lists/${listId}/items`, {
    method: 'POST',
...
export async function patchListItem(
  ...
  const res = await authJson<Data<{ item: ListItem }>>(
...
export async function deleteListItem(listId: number, itemId: number): Promise<void> {
  const res = await authFetch(`/api/v1/lists/${listId}/items/${itemId}`, { method: 'DELETE' });
  if (res.status === 204) {
    return;
  }
  await parseJsonResponse(res);
