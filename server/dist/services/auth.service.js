...
function createSession(userId) {
    ...
    db.prepare("INSERT INTO refresh_tokens (id, user_id, token_hash, expires_at, created_at) VALUES (?, ?, ?, ?, ?)").run(refreshId, userId, refreshTokenHash, refreshExpiresAt, now);
    const tokens = {
        accessToken,
        refreshToken,
...
export async function register(body) {
    ...
    const now = new Date().toISOString();
    db.prepare("INSERT INTO users (id, email, password_hash, created_at, updated_at) VALUES (?, ?, ?, ?, ?)").run(id, email, password_hash, now, now);
...
export async function refresh(refreshTokenFromBody) {
    let payload;
    try {
    ...
    const row = db
        .prepare("SELECT rt.id, rt.user_id, u.email FROM refresh_tokens rt JOIN users u ON u.id = rt.user_id WHERE rt.token_hash = ? AND rt.expires_at > datetime('now')")
        .get(tokenHash);
    if (!row) {
