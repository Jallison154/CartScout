...
const PRODUCT_SEARCH_COLUMNS =
  "id, display_name, brand, category, size_description, sold_by, image_url, source";

/** Search canonical products by display name or brand. Returns empty array if q is empty. */
export function searchProducts(q: string, limit: number = 15): Array<Record<string, unknown>> {
  ...
  return db
    ...
    .all(pattern, pattern, cappedLimit) as Array<Record<string, unknown>>;
}
